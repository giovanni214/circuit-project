...
export class Circuit {
  ...
  #computeInputLength(node, visited = new Set()) {
    ...
    if (node instanceof InputNode) {
      ...
      highest = Math.max(
        highest,
        this.#computeInputLength(node.compositeNode, visited),
      );
    }
    return highest;
  }
  ...
  evaluate(inputs = [], maxDeltaCycles = 50) {
    ...
    while (
      this.scheduler.hasEventsForTick(this.currentTick) &&
      iteration < maxDeltaCycles
    ) {
      iteration++;
  ...
  #cloneNode(node, nodeMap, preserveState = false) {
    ...
    if (node instanceof LiteralNode) {
      copy = new LiteralNode(node.value, node.name);
    } else if (node instanceof InputNode) {
      copy = new InputNode(node.index, node.name);
    } else if (node instanceof ClockNode) {
      copy = new ClockNode(node.name);
    } else if (node instanceof GateNode) {
      copy = new GateNode(node.gateType, [], node.delay, node.name);
      nodeMap.set(node, copy);
      copy.inputNodes = node.inputNodes.map((c) =>
        this.#cloneNode(c, nodeMap, preserveState),
      );
      ...
      const feedbackValue = preserveState
        ? node.currentValue
        : node.initialValue;
      ...
      const clonedInputs = node.inputNodes.map((c) =>
        this.#cloneNode(c, nodeMap, preserveState),
      );
      copy = new CompositeNode(node.subCircuit, clonedInputs, node.name);
    } else if (node instanceof SubCircuitOutputNode) {
      const clonedCompositeNode = this.#cloneNode(
        node.compositeNode,
        nodeMap,
        preserveState,
      );
      copy = new SubCircuitOutputNode(
        clonedCompositeNode,
        node.outputIndex,
        node.name,
      );
  ...
  clone(preserveState = false) {
    ...
    const roots = this.rootNodes.map((n) =>
      this.#cloneNode(n, nodeMap, preserveState),
    );
    const c = new Circuit(this.name, roots);
    c.clock = this.clock;
    c.prevClock = this.prevClock;
    c.gateRegistry = { ...this.gateRegistry };
    c.feedbackNodes = this.feedbackNodes.map((n) =>
      this.#cloneNode(n, nodeMap, preserveState),
    );
  ...
  #selectFinalPrimeCover(essentials, selectedIndices, allPrimes) {
    ...
    for (const i of selectedIndices) {
      ...
      const alreadyIncluded = final.some((ep) =>
        this.#arrayEquals(ep, candidate),
      );
      if (!alreadyIncluded) {
  ...
  #petrickMethod(minterms, primes) {
    ...
    const mintermsCoveredByEssentials =
      this.#getCoveredMintermsFromPrimes(essentialPrimes);
    const uncoveredMinterms = this.#filterUncoveredMinterms(
      minterms,
      mintermsCoveredByEssentials,
    );
    ...
    const primeCoverMatrix = this.#buildPrimeCoverMatrix(
      uncoveredMinterms,
      primes,
      essentialPrimes,
    );
    if (primeCoverMatrix.some((c) => c.length === 0)) {
      throw new Error(
        "Unsolvable: Some minterms are not covered by any remaining prime implicants.",
      );
    }

    const minimalPrimeIndices = this.#findMinimalPrimeCover(primeCoverMatrix);
    const finalPrimes = this.#selectFinalPrimeCover(
      essentialPrimes,
      minimalPrimeIndices,
      primes,
    );
  ...
  simplify() {
    ...
    for (let i = 0; i < this.outputLength; i++) {
      ...
      const andGates = bestPrimes.map((prime) => {
        ...
        prime.forEach((bit, j) => {
          if (bit === 0)
            inputsForAnd.push(new GateNode("NOT", [inputNodes[j]]));
      ...
      const simplifiedNode =
        andGates.length === 1 ? andGates[0] : new GateNode("OR", andGates);
      simplifiedNodes.push(simplifiedNode);
    }

    // Create the new circuit with all the simplified output nodes.
    const simplifiedCircuit = new Circuit(
      `${this.name}_simplified`,
      simplifiedNodes,
    );
