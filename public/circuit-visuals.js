...
function drawMinimap() {
	...
	for (let i = shapes.length - 1; i >= 0; i--) {
		...
		let scaleFactorX = totalAreaWidth > allowedWidth ? allowedWidth / totalAreaWidth : 1;
		let scaleFactorY = totalAreaHeight > allowedHeight ? allowedHeight / totalAreaHeight : 1;
	...
	for (let i = shapes.length - 1; i >= 0; i--) {
		...
		//check for right
		if (miniScreenX + miniScale > miniWidth) backgroundX = floor(-miniScreenX + miniWidth - miniScale);

		//check for bottom
		if (miniScreenY + miniScale > miniHeight) backgroundY = floor(-miniScreenY + miniHeight - miniScale);
	}
	...
	for (let i = shapes.length - 1; i >= 0; i--) {
		...
		if (miniScreenX + miniScale > miniWidth) {
			if (backgroundY === 0) {
				...
				rect(backgroundX + miniWidth, backgroundY, -backgroundX, miniHeight + -backgroundY);
			}
		}

		//check for top
		if (miniScreenY - miniScale < 0) {
			rect(backgroundX, miniScreenY - miniScale, miniWidth, -miniScreenY + miniScale);
		}
...
function mouseWheel(event) {
	...
	if (event.delta > 0) {
		// Zoom out.
		zoomFactor = zoomLevel > 4 ? baseZoomFactor + (zoomLevel - 4) * 0.1 : baseZoomFactor;
