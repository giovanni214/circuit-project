...
export class CircuitManager {
    constructor() {
        ...
        // Routing Updates
        this.anchors = []; // Replaced waypoints
        this.selectedWires = []; // Persistent wire selection
    ...
    addComponent(type, x, y, gate = null) {
        this.components.push(new VisualComponent(type, this.snap(x), this.snap(y), this.gridSize, gate));
    }
    ...
    getHoveredNode(wx, wy, radiusOverride = null) {
        ...
        // Then check if hovered over an existing Junction node
        for (const wire of this.wires) {
            for (const j of wire.junctions) {
                if (Math.hypot(wx - j.worldX, wy - j.worldY) < (radiusOverride || 10)) return j;
            }
        }
        return null;
    }
