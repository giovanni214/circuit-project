...
export class VisualComponent {
    ...
    setupNodes() {
        if (this.type === 'CIRCUIT' && this.gate) {
            ...
            this.width = Math.round(100 / this.gridSize) * this.gridSize;
            ...
            for (let i = 0; i < this.gate.outputLength; i++) {
                ...
                this.outputNodes.push(new Node(this, this.width / 2, offsetY, 'OUTPUT'));
            }
    ...
    // NEW: Hit detection specifically for the Clock Button area
    isClockHit(wx, wy) {
        if (this.type === 'CIRCUIT' && this.gate && typeof this.gate.clock !== 'undefined') {
            let btnX = this.x;
            let btnY = this.y + this.height / 2 - 12; // Matches the rendering coordinates below

            // 60px wide, 20px tall button bounding box
            return wx >= btnX - 30 && wx <= btnX + 30 && wy >= btnY - 10 && wy <= btnY + 10;
        }
        return false;
    }

    isHit(wx, wy) {
        return wx >= this.x - this.width / 2 && wx <= this.x + this.width / 2 &&
            wy >= this.y - this.height / 2 && wy <= this.y + this.height / 2;
    }
    
    // Replace the existing getNodeAt function in visual-component.js
    getNodeAt(worldX, worldY, zoom, hitRadiusOverride = null) {
        // Use the override if provided, otherwise default to the forgiving 24px radius
        const hitRadius = hitRadiusOverride !== null ? hitRadiusOverride : (24 / zoom);
    ...
    draw(font, isActive) {
        ...
        if (this.type === 'CIRCUIT' && this.gate) {
            ...
            // UPDATE: Draw a clickable button UI for the Clock
